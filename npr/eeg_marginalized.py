...
def model(n_sub, n_item, n_obs, g1, g2, treatment, obs):
    ...
    numpyro.sample('y', MarginalizedMultivariateNormalGroupCoeff(alpha + u[g1][...,0] + w[g2][...,0] + treatment * (beta + u[g1][...,1] + w[g2][...,1]), s_u, sigma, g1, treatment, n_sub, n_obs), obs=obs)
...
def main(rng_key, warm_up_steps, sample_steps):
    ...
    def recover(_, tup):
        ...
        u = jnp.zeros((n_sub, 2))
        s_u = jnp.matmul(jnp.diag(tau_u), sigma_u)
        return  None, MarginalizedMultivariateNormalGroupCoeff(alpha + u[g1][...,0] + w[g2][...,0] + treatment * (beta + u[g1][...,1] + w[g2][...,1]),
    ...
    PATH = f'result/eeg/{sample_steps}/M1'
