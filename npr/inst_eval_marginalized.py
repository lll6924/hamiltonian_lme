...
def model(n_s, n_d, n_dept, n_obs, g0, g1, g2, studage, lectage, service, obs):
    ...
    #s = numpyro.sample('s', dist.Normal(m_s,s_s), sample_shape=(n_s, 2))
    ...
    numpyro.sample('y', MarginalizedMultivariateNormalGroup(alpha + m_s + d[g1] + dept[g2] + service * ser, s_s, s_obs, g0, n_s, n_obs), obs=obs)
...
def main(rng_key, target_prob, warm_up_steps, sample_steps):
    ...
    def recover(_, tup):
        d, dept, ser, alpha, s_obs, key = tup
        m_s = 0
        return None, MarginalizedMultivariateNormalGroup(alpha + m_s + d[data["d"].values] + dept[data["dept"].values] + ser * service,
                                                         1., s_obs, data["s"].values, n_s, n_obs, m_s).sample_x(obs, key)

    keys = random.split(recover_key, len(d))
    _, sample['s'] = scan(jit(recover), None, (d, dept, ser, alpha, s_obs, keys))
    ...
    PATH = f'result/inst_eval/{sample_steps}_{target_prob}/M1'
