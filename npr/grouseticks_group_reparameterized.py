...
def model(n_brood, n_location, n_obs, g1, g2, year, height, obs):
    ...
    with numpyro.handlers.reparam(config={'brood': TransformReparam(), 'location': TransformReparam()}):
...
def main(rng_key, target_prob, warm_up_steps, sample_steps):
    ...
    PATH = f'result/grouseticks/{sample_steps}_{target_prob}/R/'
    os.makedirs(PATH, exist_ok=True)
    output_file = f'result/grouseticks/{sample_steps}_{target_prob}/R/{rng_key}'
    ...
    print(jnp.where(extra_fields["diverging"]))
