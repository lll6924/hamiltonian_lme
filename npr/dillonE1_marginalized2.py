...
def model(n_sub, n_item, n_obs, g1, g2, treatment, obs):
    ...
    numpyro.sample('y', MarginalizedMultivariateLogNormalGroupCoeff(alpha + u[g1][...,0] + w[g2][...,0] + treatment * (beta + u[g1][...,1] + w[g2][...,1]), s_w, sigma, g2, treatment, n_item, n_obs), obs=obs)
...
def main(rng_key, warm_up_steps, sample_steps):
    result = pyreadr.read_r('data/rdata/df_dillonE1.rda')
    data = result['df_dillonE1']
    ...
    data["int"] = np.array(int_encoder.fit_transform(data["int"].values))
    ...
    treatment = data["int"].astype(float).values
    print(treatment, obs)
    ...
    PATH = f'result/dillonE1/{sample_steps}/M2'
