...
def model(n_sub, n_item, n_exp, n_obs, g1, g2, g3, treatment, obs):
    ...
    numpyro.sample('y', MarginalizedMultivariateLogNormalGroupCoeff(alpha + u[g1][...,0] + w[g2][...,0] + v[g3][...,0] + treatment * (beta + u[g1][...,1] + w[g2][...,1] + v[g3][...,1]), s_w, sigma, g2, treatment, n_item, n_obs), obs=obs)
...
def main(rng_key, warm_up_steps, sample_steps):
    ...
    def recover(_, tup):
        alpha, beta, sigma, sigma_u, tau_u, sigma_w, tau_w, u, v, key = tup
        w = jnp.zeros((n_item, 2))
        s_w = jnp.matmul(jnp.diag(tau_w), sigma_w)
        return None, MarginalizedMultivariateLogNormalGroupCoeff(
            alpha + u[g1][...,0] + w[g2][...,0] + v[g3][...,0] + treatment * (beta + u[g1][...,1] + w[g2][...,1] + v[g3][...,1]),
            s_w, sigma, g2, treatment, n_item, n_obs, w).sample_x(obs, key)

    keys = random.split(recover_key, len(u))
    _, sample['w'] = scan(jit(recover), None, (alpha, beta, sigma, sigma_u, tau_u, sigma_w, tau_w, u, v, keys))
    ...
    PATH = f'result/gg05/{sample_steps}/M2'
