...
def model(n_sub, n_item, n_obs, g1, g2, treatment, obs):
    ...
    numpyro.sample('y', MarginalizedMultivariateNormalGroupCoeff(alpha + u[g1][...,0] + v[g2][...,0] + treatment * (beta + u[g1][...,1] + v[g2][...,1]), s_v, sigma, g2, treatment, n_item, n_obs), obs=obs)
...
def main(rng_key, warm_up_steps, sample_steps):
    result = pyreadr.read_r('data/rdata/df_english.rda')
    data = result['df_english']
    ...
    def recover(_, tup):
        alpha, beta, sigma, sigma_u, sigma_v, tau_u, tau_v, u, key = tup
        v = jnp.zeros((n_item, 2))
        s_v = jnp.matmul(jnp.diag(tau_v), sigma_v)

        return  None, MarginalizedMultivariateNormalGroupCoeff(alpha + u[g1][...,0] + v[g2][...,0] + treatment * (beta + u[g1][...,1] + v[g2][...,1]),
                                                         s_v, sigma, g2, treatment, n_item, n_obs, v).sample_x(obs, key)

    keys = random.split(recover_key, len(alpha))
    _, sample['v'] = scan(jit(recover), None, (alpha, beta, sigma, sigma_u, sigma_v, tau_u,tau_v, u, keys))
    to_eval = {}
    ...
    PATH = f'result/english/{sample_steps}/M2'
