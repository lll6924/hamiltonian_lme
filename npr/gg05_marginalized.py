...
def model(n_sub, n_item, n_exp, n_obs, g1, g2, g3, treatment, obs):
    ...
    numpyro.sample('y', MarginalizedMultivariateLogNormalGroupCoeff(alpha + u[g1][...,0] + w[g2][...,0] + v[g3][...,0] + treatment * (beta + u[g1][...,1] + w[g2][...,1] + v[g3][...,1]), s_u, sigma, g1, treatment, n_sub, n_obs), obs=obs)
...
def main(rng_key, warm_up_steps, sample_steps):
    ...
    def recover(_, tup):
        alpha, beta, sigma, sigma_u, tau_u, sigma_w, tau_w, w, v, key = tup
        u = jnp.zeros((n_sub, 2))
        s_u = jnp.matmul(jnp.diag(tau_u), sigma_u)
    ...
    _, sample['u'] = scan(jit(recover), None, (alpha, beta, sigma, sigma_u, tau_u, sigma_w, tau_w, w, v, keys))
    ...
    PATH = f'result/gg05/{sample_steps}/M1'
