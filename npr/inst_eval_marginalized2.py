...
def model(n_s, n_d, n_dept, n_obs, g0, g1, g2, studage, lectage, service, obs):
    ...
    numpyro.sample('y', MarginalizedMultivariateNormalGroup(alpha + s[g0] + m_d + dept[g2] + service * ser, s_d, s_obs, g1, n_d, n_obs), obs=obs)
...
def main(rng_key, target_prob, warm_up_steps, sample_steps):
    ...
    def recover(_, tup):
        s, dept, ser, alpha, s_obs, key = tup
        m_d = 0
        return None, MarginalizedMultivariateNormalGroup(
            alpha + m_d + s[data["s"].values] + dept[data["dept"].values] + ser * service,
            1., s_obs, data["d"].values, n_d, n_obs, m_d).sample_x(obs, key)

    keys = random.split(recover_key, len(s))
    _, sample['d'] = scan(jit(recover), None, (s, dept, ser, alpha, s_obs, keys))
    ...
    PATH = f'result/inst_eval/{sample_steps}_{target_prob}/M2'
