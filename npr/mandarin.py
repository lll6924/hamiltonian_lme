...
def model(n_sub, n_item, n_obs, g1, g2, treatment, obs):
    ...
    numpyro.sample('y', dist.LogNormal(alpha + u[g1][...,0] + w[g2][...,0] + treatment * (beta + u[g1][...,1] + w[g2][...,1]), sigma), obs=obs)
...
def main(rng_key, warm_up_steps, sample_steps):
    result = pyreadr.read_r('data/rdata/df_gibsonwu.rda')
    data = result['df_gibsonwu']
    ...
    data["type"] = np.array(type_encoder.fit_transform(data["type"].values))
    ...
    g1 = data["subj"].values
    g2 = data["item"].values
    print(n_obs, n_sub, n_item,)

    obs = data["rt"].astype(float).values
    treatment = data["type"].astype(float).values - 0.5
    print(treatment, obs)
    start_time = time()
    ...
    PATH = f'result/mandarin/{sample_steps}/O'
